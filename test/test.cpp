...
public:
  boost::optional<AuthenticationResponse> authenticate(const VaultClient& client) override {
    return boost::none;
  }
private:
};
...
public:
  boost::optional<AuthenticationResponse> authenticate(const VaultClient& client) override {
    return boost::optional<AuthenticationResponse>({HttpResponseBodyString{""}, Token{"success"}});
  }
private:
};
...
{
  auto response = boost::none;
  REQUIRE(!HttpClient::is_success(response));
}

TEST_CASE("HttpClient#is_success when status code not 200")
{
  auto response = boost::optional<HttpResponse>({403, HttpResponseBodyString{"Permission Denied"}});
  REQUIRE(!HttpClient::is_success(response));
}

TEST_CASE("HttpClient#is_success when status 200")
{
  auto response = boost::optional<HttpResponse>({200, HttpResponseBodyString{"OK"}});
  REQUIRE(HttpClient::is_success(response));
}
